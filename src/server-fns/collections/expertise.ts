...
import { z } from "zod";
import { expertiseItems } from "@/db/schema";
...
const list = createServerFn({ method: "GET" }).handler(async ({ context }) => {
  ...
  return genericList(expertiseItems);
});
...
const create = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    return genericCreate(expertiseItems, data, ENTITY_TYPE, admin.id, summarize);
  });
...
const update = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const admin = requireAdmin(context);
    return genericUpdate(expertiseItems, data, ENTITY_TYPE, admin.id, summarize);
  });
...
const remove = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const admin = requireAdmin(context);
    return genericRemove(expertiseItems, data.id, ENTITY_TYPE, admin.id, summarize);
  });
...
const reorder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    requireAdmin(context);
    return genericReorder(expertiseItems, data.orderedIds);
  });

export const expertiseCrud = {
  list,
  create,
  update,
  remove,
