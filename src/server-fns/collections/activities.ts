...
const create = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    return genericCreate(activities, data, ENTITY_TYPE, admin.id, summarize);
  });
...
const update = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const admin = requireAdmin(context);
    return genericUpdate(activities, data, ENTITY_TYPE, admin.id, summarize);
  });
...
const remove = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const admin = requireAdmin(context);
    return genericRemove(activities, data.id, ENTITY_TYPE, admin.id, summarize);
  });
...
const reorder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    requireAdmin(context);
    return genericReorder(activities, data.orderedIds);
  });
