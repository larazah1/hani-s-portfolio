...
const ENTITY_TYPE = "career_entry";
...
const list = createServerFn({ method: "GET" }).handler(async ({ context }) => {
  ...
  return genericList(careerEntries);
});
...
const create = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    return genericCreate(careerEntries, data, ENTITY_TYPE, admin.id, summarize);
  });
...
const update = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const admin = requireAdmin(context);
    return genericUpdate(careerEntries, data, ENTITY_TYPE, admin.id, summarize);
  });
...
const remove = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const admin = requireAdmin(context);
    return genericRemove(careerEntries, data.id, ENTITY_TYPE, admin.id, summarize);
  });
...
const reorder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    requireAdmin(context);
    return genericReorder(careerEntries, data.orderedIds);
  });

export const careerCrud = { list, create, update, remove, reorder } as unknown as CollectionCrudApi;
